...
│                                                                              │
│  ImuTask         ──→ ImuQueue ─────────────────────────────┐                 │
│  (BMI088+IST8310   AHRS融合 → Roll/Pitch/Yaw)              │                 │
│                                                             │                 │
│  PressureTask     ──→ PressureQueue ───────────────────────┤                 │
│  (MS5837 → 深度mm)                                         │                 │
│                                                             │                 │
│  RemoteTask       ──→ SbusQueue ───────────────────────────┤                 │
│  (SBUS/DBUS → 遥控通道值)                                  ├── 多源数据汇聚   │
│                                                             │                 │
│  HostTask         ──→ HostQueue ───────────────────────────┤                 │
│  (OpenMV K210 → 视觉控制指令)                               │                 │
│                                                             │                 │
...
│  (蓝牙 → 手机APP指令)                                       │                 │
│                                                             │                 │
│  RadarTask        ──→ RadarQueue ──────────────────────────┘                 │
│  (超声波 → 前后左右距离)                                                       │
└──────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│                          第 2 层：融合控制层 (ControlTask, 50ms周期)            │
│                                                                              │
│  1. 从所有 Queue 中 Peek 最新数据（不消费，下游还能读）                          │
│  2. 根据遥控器 CH5/CH7 判断当前模式（启动/定深/定姿/巡线）                       │
│  3. 计算目标值：                                                              │
│     - 位置模式 → 累加目标深度/目标偏航角                                        │
│     - 速度模式 → 直接映射摇杆量到速度指令                                       │
│  4. 生成 RovControl_t 结构体，overwrite 到 RovControlQueue                    │
└──────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│                      第 3 层：运动执行层 (MoveTask, 20ms周期)                    │
│                                                                              │
│  1. 读取 RovControl_t（目标指令）                                              │
│  2. 读取 ImuMsg_t + PressureMsg_t（当前姿态/深度反馈）                          │
│  3. 根据机型宏（ROVTYPEX/XE/I/S/F）选择推进器混合矩阵                         │
│  4. 运行 6 路 PID 控制器（深度/俯仰/横滚/偏航/巡线X/巡线Y）                      │
│     - 反馈通道经过一阶低通滤波                                                  │
│     - 输出限幅（max_out / max_iout）防饱和                                      │
│  5. 调用 DshotUpdate() 通过 DMA 输出 8 路推进器指令                             │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
| 机型宏 | 构型名称 | 推进器数量 | 控制特点 |
|--------|----------|-----------|----------|
| `ROVTYPEX` | X 标准型 | 6（4水平+2垂直） | 基础版，4推水平面 + 2推垂直面 |
| `ROVTYPEXE` | XE 增强型 | 8（4水平+4垂直） | **当前主力机型**，4垂直推参与横滚/俯仰控制 |
| `ROVTYPEI` | I 型 | 6（2水平+4垂直） | 后推禁用，前推兼做偏航，适合狭窄水域 |
| `ROVTYPES` | S 竞速型 | 8（4水平+4垂直） | 竞速优化，速度限制=1.0 |
| `ROVTYPEF` | F 迷你型 | 4（2水平+2垂直） | 极简构型，无横滚/俯仰闭环 |
